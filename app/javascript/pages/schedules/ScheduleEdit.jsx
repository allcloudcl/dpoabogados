...
class ScheduleEdit extends React.Component {
    ...
    axios
      ...
      .then((response) => {
        this.setState(
        {
          ...
          start_date: dayjs(response.data.start).format("YYYY-MM[-]DD"),
          start_time: dayjs(response.data.start).format("HH:mm"),
          end_date: dayjs(response.data.end).format("YYYY-MM-DD"),
          end_time: dayjs(response.data.end).format("HH:mm"),
        })
      })
      .catch((error) => {
        console.log(error);
  ...
  doUpdateSchedule = (e) => {
    ...
    this.props
      .dispatch(updateSchedule(Object({ schedule: schedule })))
  ...
  render() {
    return (
      <>
        ...
        <form onSubmit={this.doUpdateSchedule} className="row g-3 mt-3">
          ...
          <div>
            ...
            <LocationSelect updateLocation={this.updateLocation} commune_code={this.state.schedule.location} />
          </div>
